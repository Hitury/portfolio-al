...
export default function Experience() {
  return (
    <section id="experience" className="mx-auto w-full max-w-5xl scroll-mt-40 sm:scroll-mt-28 px-5 pt-18 pb-32">
      ...
      <ol>
        {experience.map((item) => (
          <li key={`${item.role}-${item.period}`} className="group grid sm:grid-cols-[9rem_1fr] sm:gap-10">
            <p className="text-text/55 hidden pt-1 text-xs font-medium tracking-[0.2em] uppercase sm:block">
              {item.period}
            </p>

            {/* Timeline rail: each item draws its own segment; the last one fades out. */}
